...
// Note: GoogleGenAI initialization is now performed inside each function call
// to ensure the latest API key from process.env.API_KEY is utilized, 
// as per the @google/genai guidelines for dynamic API key selection.
...
export const analyzeBusinessLogic = async (inventory: ProductGroup[], sales: Sale[]): Promise<string> => {
  try {
    ...
    // Fix: Direct property access on .text as per @google/genai guidelines (not a method call).
...
export const getMarketUpdates = async (): Promise<{ text: string, sources: { title: string, uri: string }[] }> => {
  try {
    // Fix: Initialize GoogleGenAI right before making an API call.
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    ...
    // Fix: Direct property access on .text.
    const text = response.text || "তথ্যাবলী পাওয়া যায়নি।";
    
    // Fix: Extracting search grounding URLs from groundingMetadata as per guidelines.
